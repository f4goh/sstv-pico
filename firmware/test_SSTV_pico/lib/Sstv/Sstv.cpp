...
//void Sstv::toneUs(float freq, uint32_t len) {
    ...
    // programme l'alarme une seule fois
    timer_hw->alarm[ALARM_NUM] = timer_hw->timerawl + len;
                ...
                switch (mode.tones[i].tt) {
                    case(SCAN_Y_EVEN):
                        for (uint16_t j = 0; j < mode.width; j++) {
                            if (imgtype == PROG_MEM) color = addPgm[idxLine + j] >> 8; //mire
                    ...
                    case(SCAN_R_Y):
                        for (uint16_t j = 0; j < mode.width / 2; j++) {
                            //color=128;
                            if (imgtype == PROG_MEM) color = addPgm[idxLine + j * 2 + 1] & 0xff;
                    ...
                    case(SCAN_Y_ODD):
                        for (uint16_t j = 0; j < mode.width; j++) {
                            if (imgtype == PROG_MEM) color = addPgm[idxLine + j] >> 8;
                    ...
                    case(SCAN_B_Y):
                        for (uint16_t j = 0; j < mode.width / 2; j++) {
                            //color=128;
                            if (imgtype == PROG_MEM) color = addPgm[idxLine + j * 2] & 0xff;
